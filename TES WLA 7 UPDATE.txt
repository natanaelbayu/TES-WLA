...
st.markdown("""
    <div class="main-header">
        ...
        <p>Metode: Time Study & Workload Analysis (WLA) - Single Line Production</p>
    </div>
""", unsafe_allow_html=True)
...
threshold_underload = st.sidebar.number_input(
    "Batas Maksimal Underload (%)", value=100.0
)
threshold_overload = st.sidebar.number_input(
    "Batas Minimal Overload (%)", value=128.0
)
...
with tab2:
  ...
  fig_bar.add_hline(
      ...
      line_dash="dash",
      line_color="#EAA8BA",
      annotation_text="Batas Underload (<100%)",
  )
  fig_bar.add_hline(
      y=threshold_overload,
      line_dash="dash",
      line_color="#561D47",
      annotation_text="Batas Overload (>128%)",
  )
...
with tab3:
  ...
  if list_over and list_under:
    ...
    st.caption(
        ...
        f" {op_penerima} tidak menjadi Overload > 128%)."
    )
